...
% Worhshop on Algorithms in Bioinformatics (WABI), 2011. Jonathan Qiang Jiang
...
for lab_idx=1:length(LabRatios)
    ...
    for run=1:round
        ...
        P_np=[(1-beta)*P_p, beta*P_pf;beta*P_pf',(1-beta)*P_f];
        ...
        fprintf('==PfunBG_CK Run=%d,lab_ratio=%-10.4f,Nfun2=%d, time:%s\n',run,lab_ratio,Nfun2,datestr(now));
...
evalstr=['save D:\TMEC\TMEC_Web\results',filesep,dataname, '_PfunBG_CK_Ls.mat LabRatios precision stds prec_seq'];
eval(evalstr);   

%evalstr=['save results',filesep,dataname, '_PfunBG_CK_Ls.mat gamma beta LabRatios precision stds prec_seq'];
%eval(evalstr);

%fprintf('\n =====%s finish PfunBG_CK_Ls time=%s\n',dataset,datestr(now));
